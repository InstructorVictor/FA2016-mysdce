(function () {
    ...
    function onDeviceReady() { 
		...
		// Store various Device information
		var osVer = device.version,
			osVerInt = parseInt(osVer, 10),
			osName = device.platform;
		
		// Check which Device is running, in order to Show/Hide features of the app
		switch(osName) {
			case "Android":
				if(osVerInt >= 4) {
					console.log("Android OS greater than 4: " + osVerInt);
				} else {
					console.log("Android OS less than 4: " + osVerInt);
					$("#btnClasses").hide();
				}
				break;
			case "iOS":
		...
		$("#btnEmail").on("click", fnEmail);
		function fnEmail() {
			window.plugins.socialsharing.shareViaEmail(
				"Regarding your app<br>", 		//"Message Body",
				"mySDCE Feedback", 				//"Subject", 
				["vcampos@sdccd.edu"], 			//["To:"], // Requires an Array of Strings
				null, 							//["CC:"], // Requires an Array of Strings
				null, 							//["BCC:"], // Requires an Array of Strings
				"www/images/ce-logo-vert.png", 	//"Attachments", // Requires www/ in path of Local
				function(result){console.log("Success: " + result);}, //"Success", // An inline Anonymous Function or a named Function
				function(error){console.log("Failure: " + error);}//"Fail"
			);
		} // END fnEmail()
		
		$("#btnShare").on("click", fnShare);
		function fnShare() {
			window.plugins.socialsharing.share(
				"Check out the mySDCE app! ", // Message Body,
				"Download the mySDCE app ", // Message Subject,
				["www/images/org_image.jpg"], // Attachments as string, in an Array,
				"http://a.co/80zPCEU", // A URL as a String. If unused, include null,
				function(result){console.log("Success: " + result);}, //"Success", // An inline Anonymous Function or a named Function
				function(error){console.log("Failure: " + error);}// Fail Callback (no final comma)
			);
		} // END fnShare()
